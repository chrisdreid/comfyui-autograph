...
- **V3 widget classification** — inputs declared as `["BOOLEAN", {}]` / `["INT", {}]` / etc. with an empty options dict (common in V3 node schemas, e.g. `PrimitiveBoolean.value`, `ComfySwitchNode.switch`) were misclassified as connection-only and dropped from the API prompt, producing `required_input_missing` node_errors that ComfyUI reports inside an HTTP 200 while silently skipping the dependent output nodes. Widget-vs-connection classification now follows the schema's own signals: widget-capable type names (INT/FLOAT/BOOLEAN/STRING/COMBO and dynamic combos), a `socketless` key (custom widget types like COLOR/CURVE/editor states), or a `widgetType` override (multi-type widgets like `"FLOAT,INT"`); everything else — `COMFY_MATCHTYPE_V3`, `COMFY_AUTOGROW_V3`, plain data types, unmarked multi-types — is a connection. This also fixes tooltip-only widgets (e.g. `SaveLoRA.steps`) that were previously dropped, and stops phantom widgets from shifting value alignment.
...
- **Chained subgraph instances dropped connections** — when one subgraph instance's output fed another instance's input, the link created while flattening the first instance was invisible to the second (stale link lookup), silently severing the connection (e.g. the official Qwen InstantX ControlNet template lost `ControlNetApplyAdvanced.image`). The link lookup now tracks links created during the same flatten pass.
- **Null-padded instance `widgets_values`** — some frontend versions serialize subgraph instance widget values with `null` placeholder slots (e.g. `[null, null, null, 1]` for one promoted widget), which mapped the padding onto real widgets (`strength: null`). Null slots are now filtered out, and when counts still don't line up, values are only assigned while they fit the subgraph input's declared type (falling back to the inner nodes' own values).
